...
class ConfigFeature(FeatureBaseResource):
    ...
    kitsu_postgres_conf_str: str = Field(
        description="The Kitsu Postgres configuration file.",
        default=POSTGRES_CONF_STR,
        exclude=True,
    )
    ...
    #     ret = pathlib.Path(
    #         pathlib.Path(self.kitsu_database_install_destination).expanduser()  # pylint: disable=E1101
    #         .as_posix()
    #         .format(
    #             **{
    #                 "FEATURE": self.feature_name,
    ...
    #     ret = pathlib.Path(
    #         pathlib.Path(self.kitsu_preview_folder).expanduser()  # pylint: disable=E1101
    #         .as_posix()
    #         .format(
    #             **{
    #                 "FEATURE": self.feature_name,
    ...
    #     ret = pathlib.Path(
    #         pathlib.Path(self.kitsu_tmp_dir).expanduser()  # pylint: disable=E1101
    #         .as_posix()
    #         .format(
    #             **{
    #                 "FEATURE": self.feature_name,
...
config_yaml: pathlib.Path = get_absolute_config_path(dist).joinpath("config.yml")


config_yaml.parent.mkdir(parents=True, exist_ok=True)
if not config_yaml.exists():
    dump_yaml(
        model_config=ConfigFeature(),
        file_path=config_yaml,
    )
_yaml: Dict = load_yaml(file_path=config_yaml)
json_str = json.dumps(_yaml, indent=2)
config_feature: ConfigFeature = ConfigFeature.model_validate_json(json_str)
